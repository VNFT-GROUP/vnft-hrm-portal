    ...
    <>
      ...
      {formState === 'login' && (
        <form onSubmit={handleLoginSubmit} className="login-form fade-in" autoComplete="off">
          <div className="field">
            <label htmlFor="login-email" className="field-label">{t('login.emailLabel', "Email / Tên đăng nhập")}</label>
          ...
          <div className="field">
            <div className="field-label-row">
              <label htmlFor="login-password" className="field-label">{t('login.passwordLabel', "Mật khẩu")}</label>
            ...
            <div className="field-input-wrap">
              ...
              <button type="button" className="password-toggle" onClick={() => setField('showPassword', !state.showPassword)}>
                {state.showPassword ? <EyeOff /> : <EyeOpen />}
              </button>
            </div>
          </div>

          <button type="submit" className={`submit-btn${state.loading ? " submit-btn--loading" : ""}`} disabled={state.loading}>
            {state.loading ? <span className="btn-spinner" /> : <><span>{t('login.submit', "Đăng Nhập")}</span><ArrowRight /></>}
          </button>
        </form>
      )}
      ...
      {formState === 'forgot_email' && (
        <form onSubmit={handleForgotEmailSubmit} className="login-form fade-in" autoComplete="off">
          <div className="field">
            <label htmlFor="forgot-email" className="field-label">{t('login.emailLabel', "Email Nhân Viên")}</label>
            <div className="field-input-wrap">
              <svg className="field-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <button type="submit" className={`submit-btn${state.loading ? " submit-btn--loading" : ""}`} disabled={state.loading}>
            {state.loading ? <span className="btn-spinner" /> : <><span>{t('login.sendOtp', "Gửi OTP")}</span><ArrowRight /></>}
          </button>
      ...
      {formState === 'forgot_otp' && (
        <form onSubmit={handleOtpSubmit} className="login-form fade-in" autoComplete="off">
          <div className="field">
            <label className="field-label">{t('login.otpLabel', "Mã OTP")}</label>
          ...
          <button type="submit" className={`submit-btn${state.loading ? " submit-btn--loading" : ""}`} disabled={state.loading}>
            {state.loading ? <span className="btn-spinner" /> : <><span>{t('login.verifyOtp', "Xác nhận OTP")}</span><ArrowRight /></>}
          </button>
          <button type="button" className="btn-back" onClick={() => setFormState('login')} disabled={state.loading}>{t('login.backToLogin', "Quay lại đăng nhập")}</button>
        </form>
      )}
      ...
      {formState === 'forgot_reset' && (
        <form onSubmit={handleResetPasswordSubmit} className="login-form fade-in" autoComplete="off">
          <div className="field">
            <label htmlFor="new-password" className="field-label">{t('login.newPasswordLabel', "Mật khẩu mới")}</label>
            <div className="field-input-wrap">
              <svg className="field-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" />
              </svg>
              ...
              <button type="button" className="password-toggle" onClick={() => setField('showNewPassword', !state.showNewPassword)}>
                {state.showNewPassword ? <EyeOff /> : <EyeOpen />}
              </button>
            </div>
          </div>
          <div className="field">
            <label htmlFor="confirm-password" className="field-label">{t('login.confirmPasswordLabel', "Xác nhận mật khẩu")}</label>
            <div className="field-input-wrap">
              <svg className="field-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <button type="submit" className={`submit-btn${state.loading ? " submit-btn--loading" : ""}`} disabled={state.loading}>
            {state.loading ? <span className="btn-spinner" /> : <><span>{t('login.updatePassword', "Cập nhật mật khẩu")}</span><ArrowRight /></>}
          </button>
          <button type="button" className="btn-back" onClick={() => setFormState('login')} disabled={state.loading}>{t('login.backToLogin', "Quay lại đăng nhập")}</button>
        </form>
      )}
