    ...
    <div className="w-full relative min-h-[400px] border border-slate-200 rounded-xl overflow-hidden shadow-sm flex flex-col bg-white">
      <div className="overflow-x-auto flex-1 custom-scrollbar">
        <table className="w-full text-sm text-left whitespace-nowrap">
          <thead className="text-[13px] text-slate-500 uppercase bg-slate-50/80 sticky top-0 z-10 
            ...
            <tr>
              <th className="px-5 py-4 font-bold tracking-wider text-[#1E2062]">{t("reports.columns.empId", { defaultValue: "Mã NV" })}</th>
              <th className="px-5 py-4 font-bold tracking-wider text-[#1E2062]">{t("reports.columns.fullName", { defaultValue: "Họ Tên" })}</th>
              <th className="px-5 py-4 font-bold tracking-wider text-[#1E2062]">{t("reports.columns.department", { defaultValue: "Phòng Ban" })}</th>
              <th className="px-5 py-4 font-bold tracking-wider text-[#1E2062] text-center border-l border-slate-200/50">{t("reports.columns.performance", { defaultValue: "Hiệu Suất" })}</th>
              <th className="px-5 py-4 font-bold tracking-wider text-[#1E2062] text-center">{t("reports.columns.attendance", { defaultValue: "Chuyên Cần" })}</th>
              <th className="px-5 py-4 font-bold tracking-wider text-[#1E2062] text-right border-l border-slate-200/50">{t("reports.columns.pcPerformance", { defaultValue: "PC Hiệu Suất" })}</th>
              <th className="px-5 py-4 font-bold tracking-wider text-[#1E2062] text-right">{t("reports.columns.pcAttendance", { defaultValue: "PC Chuyên Cần" })}</th>
              <th className="px-5 py-4 font-extrabold tracking-wider text-rose-700 text-right bg-rose-50/50 border-l border-rose-200/50">{t("reports.columns.totalAllowance", { defaultValue: "Tổng Phụ Cấp" })}</th>
            </tr>
          </thead>
              ...
              <tr>
                <td colSpan={8} className="h-64 text-center">
                  <div className="flex flex-col items-center justify-center text-slate-400">
                    ...
                    <p className="text-base font-semibold text-slate-800">{t("reports.table.noAllowanceData", { defaultValue: "Không có dữ liệu phụ cấp cho kỳ đã chọn" })}</p>
      ...
      {items.length > 0 && (
        <div className="px-5 py-3 border-t border-slate-200 bg-slate-50 flex justify-between items-center shrink-0">
          <span className="text-sm font-medium text-slate-500">{t("reports.table.totalEmpCount", { count: items.length, defaultValue: `Tổng cộng ${items.length} nhân viên` })}</span>
        </div>
      )}
    </div>
  );
