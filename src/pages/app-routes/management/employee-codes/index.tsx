...
export default function EmployeeCodesPage() {
  ...
  const updateDescMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Cáº­p nháº­t mÃ´ táº£ thÃ nh cÃ´ng');
      queryClient.invalidateQueries({ queryKey: ['employee-codes'] });
      setIsOpen(false);
    },
  });
    ...
    <div className='p-4 md:p-8 w-full min-h-full flex flex-col gap-6 md:gap-8'>
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className='flex flex-col gap-2'>
        <h1 className='text-2xl md:text-4xl font-bold text-[#1E2062] flex items-center gap-3'>
          ...
          {t('management.employeeCodesTitle', { defaultValue: 'Quáº£n lÃ½ MÃ£ nhÃ¢n viÃªn' })}
        </h1>
        <p className='text-muted-foreground text-base md:text-lg ml-1'>
          {t('management.employeeCodesDesc', { defaultValue: 'Thiáº¿t láº­p cáº¥u hÃ¬nh tiá»n tá»‘ mÃ£ nhÃ¢n viÃªn (Prefix)' })}
        </p>
      </motion.div>
        ...
        >
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3 w-full">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{t('management.toggleLegend', { defaultValue: 'Báº­t / Táº¯t hoáº¡t Ä‘á»™ng' })}</span>
            </div>
      ...
      <motion.div className='bg-card p-5 rounded-2xl shadow-sm border border-border flex flex-col md:flex-row justify-between items-center gap-4'>
        <div className='relative w-full md:w-96'>
          ...
          <Input placeholder={t('management.searchPrefixPlaceholder', { defaultValue: 'TÃ¬m kiáº¿m prefix...' })} className='pl-12 h-12 rounded-xl bg-muted border-border focus-visible:ring-[#2E3192] hover:bg-card transition-colors' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        <Button onClick={() => handleOpenForm()} className='w-full md:w-auto h-12 px-6 rounded-xl bg-[#2E3192] hover:bg-[#1E2062] text-white shadow-[#2E3192]/20 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300'>
          <Plus size={20} className='mr-2' /> {t('management.addNew', { defaultValue: 'Táº¡o má»›i' })}
        </Button>
      </motion.div>
